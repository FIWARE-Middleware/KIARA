...
-   Version: [0.3.0](#)
-   Previous version:
    [0.2.0](Middleware_RPC_API_Specification_0.2.0.html)
...
-   **getTypeDescriptorBuilder**: This function returns an instance of
    ...
    described
    [*here*](./Middleware_RPC_Dynamic_Types_API_Specification.html).
...
-   **shutdown**: This function closes and releases all internal
    Advanced Middleware structures (e.g. stops all pending tasks). Call
    this before you exit your application.
...
-   **connect**: This function creates a new connection to the server.
    This connection might be used by proxies to send requests to
    the server.
-   **createTransport**: This function provides a direct way to create a
    ...
    specific use cases.
-   **createSerializer**: This function provides a direct way to create
    a specific `Serializer` instance which can be configured for
    specific use cases.
-   **createServer**: This function creates a new `Server` instance used
...
developer must use the factory method `createTransport` of the interface
*org.fiware.kiara.Context*, which will return a compliant network
...
-   **getTransportFactory**: This function returns an instance of a
    factory class which was used to create this server
    transport instance.
...
-   **isRunning**: Returns true if server is up and waiting for
    incoming connections.
-   **startServer**: Starts server.
-   **stopServer**: Stops server.
-   **getLocalTransportAddress**: Returns transport address to which
...
-   **register:** Register a `Servant` object or `DynamicHandler` with
    the service.
...
-   **getServiceName**: Returns the name of the service implemented by
    this servant.
-   **process**: This function processes the incoming request message
    and returns the produced response message. It is
    automatically generated.

### Dependent API

This subsection contains the interfaces and classes that are dependent
from the user Advanced Middleware IDL definition. In the static version
of the Advanced Middleware implementation these interfaces and classes
should be generated by the compile time preprocessor.\
...
x.y.&lt;IDL-ServiceInterface&gt;Async. The Advanced Middleware IDL
service proxy will implement this interface, allowing the user to call
the service’s remote procedures synchronously or asynchronously. It is
only used on the client side in order to make the Proxy to implement all
the functions for this service (both synchronous and asynchronous).

**Functions:**

-   **add**: Function inherited from
    *x.y.&lt;IDL-ServiceName&gt;* interface. This function is the
    mapping of the Advanced Middleware IDL service.
-   **add**: Function inherited from
    *x.y.&lt;IDL-ServiceName&gt;Async* interface. This function is the
    asynchronous version of the Advanced Middleware IDL
    service’s procedure.
...
-   **add**: Function inherited from
    *x.y.&lt;IDL-ServiceName&gt;Client* interface. This function is the
    mapping of the Advanced Middleware IDL service.
-   **add**: Function inherited from
    *x.y.&lt;IDL-ServiceName&gt;Client* interface. This function is the
    asynchronous version of the Advanced Middleware IDL
...
from the interface *x.y.&lt;IDL-ServiceName&gt;* leaving the
implementation of this functions to the user.
...
### Network transports
  ...
  ------------------------------------------------------------------------
  **org.fiware.kiara.transport.Tran                              
  sport**                                                        
  --------------------------------- ------------ --------------- ---------
  **Attributes**                                                 
  ...
  *Name*                            *Parameters* *Returns/Type*  *Raises*
  ...
  getTransportFactory                            TransportFactor 
                                                 y               
  ------------------------------------------------------------------------
...
section [*API Examples*](#api-examples) to define them.

  -------------------------------------------------------------------------
  **x.y.&lt;IDL-ServiceName&g                                   
  t;**                                                          
  --------------------------- --------------- ----------------- -----------
  **Attributes**                                                
  ...
  *Name*                                     *Parameter *Returns/Ty *Raises
  ...
  Inherited from x.y.&lt;IDL-ServiceName&gt;                        
  and x.y.&lt;IDL-ServiceName&gt;Async                              
  -------------------------------------------------------------------------
  ...
  *Name*                             *Type*                           
  ...
  m\_ser                             org.fiware.kiara.seria           
                                     lization.Serializer              

  m\_transport                       org.fiware.kiara.trans           
                                     port.Transport                   

  **Public Operations**                                               

  *Name*                             *Parameters*           *Returns/ *Raise
                                                            Type*     s*

  Inherited from                                                      
  x.y.&lt;IDL-ServiceName&gt; and                                     
  x.y.&lt;IDL-ServiceName&gt;Async                                    
  --------------------------------------------------------------------------
...
Middleware IDL*](./Middleware_IDL_Specification.html):
...
`Transport` and `Serialization` instances are implizitly created by the
connection, based on the string parameter of the `connect` method.
...
`Transport` and `Serialization` instances are implizitly created by the
connection, based on the string parameters of the `addService` method.
