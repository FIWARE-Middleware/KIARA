	...
	public TransportMessage process(Serializer ser, Transport transport, TransportMessage message, Object messageId) {
		...
		if (operation.equals("function")) {
			...
			try {
				...
				// Serialize return code (non-zero error code)
				serImpl.serializeUI32(retMsg, "", 0x29E1B589);
				// Serialize exception name
				serImpl.serializeString(retMsg, "", "FirstException");
				// Serialize exception data
				ex_FirstException.serialize(serImpl, retMsg, "");
			}

			catch(SecondException ex_SecondException) {
				ex_SecondException.printStackTrace();
				// Serialize return code (non-zero error code)
				serImpl.serializeUI32(retMsg, "", 0xD8044958);
				// Serialize exception name
				serImpl.serializeString(retMsg, "", "SecondException");
				// Serialize exception data
				ex_SecondException.serialize(serImpl, retMsg, "");
			}
