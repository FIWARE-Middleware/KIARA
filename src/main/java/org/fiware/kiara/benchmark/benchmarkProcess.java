...
public class benchmarkProcess {
	...
	public static void sendMarketData_processAsync(final TransportMessage message, final SerializerImpl ser, final AsyncCallback<MarketData> callback) {
	    try {
	        ...
	        if (retCode == 0) { // Function execution was OK.
	            MarketData result = ser.deserialize(bis, "", MarketData.class);
	...
	public static void sendQuoteRequest_processAsync(final TransportMessage message, final SerializerImpl ser, final AsyncCallback<QuoteRequest> callback) {
	    try {
	        final BinaryInputStream bis = BinaryInputStream.fromByteBuffer(message.getPayload());
	        // Deserialize message ID
	        final Object messageId = ser.deserializeMessageId(bis);
	        ...
	        int retCode = ser.deserializeUI32(bis, "");
	        if (retCode == 0) { // Function execution was OK.
	            QuoteRequest result = ser.deserialize(bis, "", QuoteRequest.class);
	            callback.onSuccess(result);
	        }
	    } catch (Exception ex) {
	        callback.onFailure(ex);
